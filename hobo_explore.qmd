...
shelf(tidyverse, here, DBI, RPostgres, janitor, readxl, parsedate, lubridate, zoo)
...
for(i in 1:n_distinct(filenames)){
  ...
  csv <- read_csv(here("raw_data", paste(filenames[i], ".csv", sep="")), skip = 1) %>% 
    ...
    ungroup() %>% 
    mutate(twoweek_temp_roll_avg = rollmean(mean_daily_temp_c, 14, fill = list(NA, NULL, NA), align = "right"))
...
for (i in 1:n_distinct(filenames)) {
  ...
  p<- ggplot(data = data) +
        geom_line(aes(x=date, y=twoweek_temp_roll_avg)) +
